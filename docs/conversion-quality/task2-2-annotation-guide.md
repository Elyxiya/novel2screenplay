...
| `zhanlan-long`（中段 180–269 章） | long | ✅ 标注骨架（90 章，≈58.8k token 超 30k 截断上限；真实死亡 1 起「白斯」切片第 9 章，揭示如实空，供长样本 mapreduce 验证；坐标=切片内 1 起） |

**余量缺口**：short / medium 均已凑满（3 短 + 3 中），已达标 Task 2.2 的 3–5 短 + 3–5 中要求；long 非 2.2 强制项，已按扩展补 1 本长样本骨架。后续如需扩展可再补 1–2 本，优先公共版权/自创作文本，原则：**有真实死亡/揭示事件的切片优先**。
