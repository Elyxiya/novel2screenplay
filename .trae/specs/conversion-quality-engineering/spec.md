...
13. **旧路径留 flag**：Phase1 旧截断路径保留做 baseline，步骤间对比数字在其产物上量出；翻默认后再整体清掉 flag（另走一次独立 PR）。**【压后：flag 清理显式晚于 T2-C5 出数之后】**——旧路径是分层对比（T2.5）唯一 baseline 来源，删除前量不出曲线，故旧路径清理独立 PR 必须排在 T2-C5 出数之后。
